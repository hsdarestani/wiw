...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await sendPush(recipients.map((recipient) => recipient.userId), "Neue Nachricht", `${user.firstName} ${user.lastName}: ${parsed.data.content.slice(0, 120)}`, { type: "MESSAGE", conversationId: id });
