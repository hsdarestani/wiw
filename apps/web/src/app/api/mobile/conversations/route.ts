...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: "Bitte Mitglieder auswählen." }, { status: 400 });
  ...
  if (members.length !== ids.length) return NextResponse.json({ error: "Mitglied ungültig." }, { status: 400 });
  if (ids.length === 2) {
