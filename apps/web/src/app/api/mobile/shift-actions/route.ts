...
const schema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("CLAIM_OPEN"), shiftId: z.string().min(1) }),
  z.object({ action: z.literal("OFFER_SWAP"), shiftId: z.string().min(1) }),
  z.object({ action: z.literal("CLAIM_SWAP"), tradeId: z.string().min(1) }),
  z.object({
    action: z.literal("REVIEW"),
    tradeId: z.string().min(1),
    decision: z.enum(["APPROVED", "DECLINED"]),
  }),
  z.object({ action: z.literal("CANCEL"), tradeId: z.string().min(1) }),
]);
...
export async function POST(request: Request) {
  ...
  if (input.action === "CLAIM_OPEN") {
    ...
    const duplicate = await prisma.shiftTrade.findFirst({
      where: {
        shiftId: shift.id,
        claimantId: user.id,
  ...
  if (input.action === "CANCEL") {
    if (trade.ownerId !== user.id && trade.claimantId !== user.id)
      return NextResponse.json({ error: "Nicht erlaubt." }, { status: 403 });
    if (!new Set(["OFFERED", "PENDING"]).has(trade.status))
      return NextResponse.json(
        { error: "Anfrage ist abgeschlossen." },
        { status: 409 },
      );
    const updated = await prisma.shiftTrade.update({
      where: { id: trade.id },
